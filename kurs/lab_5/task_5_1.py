...
Italian speakers trying to pronounce it :(So, if we are to start our Tribonacci sequence with [1, 1, 1]
as a starting input (AKA signature), we have this sequence: [1, 1 ,1, 3, 5, 9, 17, 31, ...].
...
sequence shifted by 2 places, but that is not the case and we would get: [0, 0, 1, 1, 2, 4, 7, 13, 24, ...].
...
def tri_bon(f_num, s_num, th_num, len_list):
    ...
    res = [int(i) if int(i) == i else i for i in res]
    for _ in range(3, len_list):
        temp = sum(res[-3:])
        temp = int(temp) if int(temp) == temp else round(temp, 2)
        res.append(temp)
    return res
